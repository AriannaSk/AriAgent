...
public class DzivoklisService : IDzivoklisService
{
  ...
  private async Task<bool> ApartmentNumberExistsAsync(Guid majaId, int numurs, Guid? excludeId = null)
  {
    return await _db.Dzivokli.AnyAsync(d =>
        d.MajaId == majaId &&
        d.Numurs == numurs &&
  ...
  {
    var apartmentExists = await ApartmentNumberExistsAsync(dzivoklis.MajaId, dzivoklis.Numurs);

    if (apartmentExists)
      throw new InvalidOperationException("Apartment with this number already exists in this house.");
  ...
  public async Task<bool> UpdateAsync(Guid id, Dzivoklis dzivoklis, List<Guid> iedzivotajsIds)
  {
    ...
    var apartmentExists = await ApartmentNumberExistsAsync(
        dzivoklis.MajaId,
        dzivoklis.Numurs,
        id
    );
